...
func TestShouldCompactProactivelyHonorsExplicitReserve(t *testing.T) {
	...
	if a.shouldCompactProactively("claude-opus-4-8", 905_000) {
		t.Error("explicit reserve must be honored, not inflated by the window fraction")
	}
	if !a.shouldCompactProactively("claude-opus-4-8", 995_000) {
...
func TestShouldCompactProactivelyDisabled(t *testing.T) {
	if (&Agent{Config: &Config{ContextWindow: -1}}).shouldCompactProactively("claude-opus-4-8", 999_999_999) {
		t.Error("a negative context window disables proactive compaction")
	}
	if (&Agent{Config: &Config{}}).shouldCompactProactively("claude-opus-4-8", 0) {
