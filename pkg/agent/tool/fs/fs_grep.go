	...
	return tool.Tool{
		...
		Parameters: map[string]any{
			...
			"properties": map[string]any{
				...
				"path":    map[string]any{"type": "string", "description": "File or directory to search in (rg PATH). Defaults to workspace."},
				"glob":    map[string]any{"type": "string", "description": "Glob pattern to filter files (e.g. `*.js`, `*.{ts,tsx}`) — maps to rg --glob."},
				"type":    map[string]any{"type": "string", "description": "File type to search (rg --type). Common types: js, py, rust, go, java, etc. More efficient than `glob` for standard file types."},
				"output_mode": map[string]any{
					"type":        "string",
					"description": "Output mode. `content` shows matching lines (supports -A/-B/-C context, -n, head_limit), `files_with_matches` shows file paths (supports head_limit), `count` shows match counts. Defaults to `files_with_matches`.",
