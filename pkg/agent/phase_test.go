...
func TestSendHonorsProviderEndTurn(t *testing.T) {
	unphased := strings.ReplaceAll(commentaryOutput, `"phase":"commentary",`, "")
	for _, tc := range []struct {
		name         string
		endTurn      string
		output       []string
		...
		{"false with unphased text", "false", []string{unphased}, 2},
		{"false without output", "false", nil, 2},
		{"false with final phase", "false", []string{finalAnswerOutput}, 2},
		{"true with commentary", "true", []string{commentaryOutput}, 1},
		{"true with final phase", "true", []string{finalAnswerOutput}, 1},
		{"true with tool call", "true", []string{`{"type":"function_call","id":"fc_check","call_id":"call_check","name":"check","arguments":"{}","status":"completed"}`}, 2},
		{"absent with commentary", "", []string{commentaryOutput}, 2},
		{"null with commentary", "null", []string{commentaryOutput}, 2},
		{"null with unphased text", "null", []string{unphased}, 1},
	} {
		t.Run(tc.name, func(t *testing.T) {
			requests := 0
			client := streamingTestClient(func(*http.Request) string {
				...
				stream := phaseTestResponse(false, tc.output...)
				if tc.endTurn != "" {
					stream = strings.ReplaceAll(stream, `"response":{`, `"response":{"end_turn":`+tc.endTurn+`,`)
				}
				return stream
			})
			a := &Agent{Config: &Config{
