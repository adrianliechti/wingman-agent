...
func (a *App) clearChat() {
	...
	// state stays around in the coder.Agent map but the UI is
...
// estimatedStreamOutputTokens approximates the output tokens generated by the
// in-flight request from the text/reasoning streamed so far (~4 chars/token).
// Returns 0 when not streaming so the committed usage is shown verbatim.
...
func (a *App) updateStatusBar() {
	...
	// The API only reports usage once a request completes, so a long
	// in-flight response would otherwise show a frozen count. While
	// streaming, fold in a rough estimate of the output tokens produced so
	// far (marked with ~) so the counter moves; it's replaced by the exact
	// value the moment the turn lands.
