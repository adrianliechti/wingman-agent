...
// Overlay is a full-screen alternate-buffer view. HandleKey returns true when
...
// transcriptOverlay shows the full session, tools expanded.
...
func (a *App) showTranscript() {
	...
	for _, msg := range a.agent.Messages(a.sessionID) {
		...
		for _, c := range msg.Content {
			...
			case c.ToolResult != nil:
				...
				lines = append(lines, cellToolTranscript(c.ToolResult, width)...)
			case c.Reasoning != nil && c.Reasoning.Summary != "":
				lines = append(lines, cellReasoning(c.Reasoning.Summary, width, true, false)...)
				lines = append(lines, "")
			case c.Text != "":
				switch msg.Role {
				case agent.RoleUser:
	...
	o := &transcriptOverlay{
		pager: pager{
			title: "transcript",
			lines: lines,
			hints: dim("↑↓/jk scroll · g/G top/bottom · esc close"),
		},
	}
